                ...
                <TodayCtr>
                  <div className={ styles.mainInfo }>
                    ...
                    <div className={ styles.weather }>
                      <p>{ days[0].temp }</p>
                      <p>Ощущается как <span>{ days[0].feelslike }</span></p>
                    </div>
                    <p>{ getLocalizedDate(new Date(days[0].datetime)) }</p>
                  </div>

                  <WeatherIcon icon={ days[0].icon as string } caption={ days[0].conditions as string }/>

                  <div className={ styles.measures }>
                    <Measure icon="humidity" title="Влажность" value={ `${ days[0].humidity }%` }/>
                    <Measure icon="sunrise" title="Восход" value={ splitLast(days[0].sunrise, ':')[0] }/>
                    <Measure icon="sunset" title="Закат" value={ splitLast(days[0].sunset, ':')[0] }/>
                  </div>
                </TodayCtr>

                <DaysCtr>
                    { days.map(day => <DayCard day={ day } key={ day.datetime }/>) }
                </DaysCtr>
              </>
            }
        </main>
